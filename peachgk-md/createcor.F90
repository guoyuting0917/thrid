!*************************************************
!*  createcor_.f Ver.2.5Pt-Ar-Pt(111) '28.10.2014  *
!*      for peachgk_md.f                          *
!**************************************************
...
!     parameters for fcc lattice
  real(8):: A0,A1,A2!,A3
  real(8):: snx,sny,snz
  real(8):: vacum_layer,liquid_layer,liquid_adsorb

  real(8):: lower_liquid,upper_liquid
  real(8):: first_layer,second_layer,third_layer,last_layer
  
  real(8):: base_latt(2,12)
...
!---- notation ----
  ...
  A0 = 2.77d-10 / xref          ! distance of x-axis
  A1 = A0 * sqrt(3.0) * 0.5     ! distance of y-axis
  A2 = sqrt(6.0)/3.0 * A0     ! distance of z-axis
...
!------------------------z layer -------------------------
  ...
  first_layer = vacum_layer + DBLE(zmaxma(1)-1)*A2 + A2

  second_layer = first_layer +  DBLE(zmaxma(2)-1)*A2 + A2

  lower_liquid = second_layer + DBLE(zmaxma(3)-1) * A2

  upper_liquid = lower_liquid + liquid_layer + 2.0d0 * liquid_adsorb

  third_layer = upper_liquid + DBLE(zmaxma(4)-1)*A2 + A2

  last_layer = third_layer + DBLE(zmaxma(5)-1)*A2 + A2 

!---- create base lattice for Au (1 layer)
  base_latt(1,1) = 0.0d0*A0   ! x value for atom no 1
  base_latt(2,1) = 0.0d0*A1   ! y value for atom no 1
  base_latt(1,2) = 1.0d0*A0   ! x value for atom no 2
  base_latt(2,2) = 0.0d0*A1   ! y value for atom no 2 
  
  base_latt(1,3) = 0.5d0*A0
  base_latt(2,3) = 1.0d0*A1
  base_latt(1,4) = 1.5d0*A0
  base_latt(2,4) = 1.0d0*A1

  base_latt(1,5) = 0.0d0*A0
  base_latt(2,5) = 2.0d0*A1
  base_latt(1,6) = 1.0d0*A0
  base_latt(2,6) = 2.0d0*A1

  base_latt(1,7) = 0.5d0*A0
  base_latt(2,7) = 3.0d0*A1
  base_latt(1,8) = 1.5d0*A0
  base_latt(2,8) = 3.0d0*A1

!---- shift amount of base lattice for Au
  shift_latt(1,0) = 0.0d0*A0          !x value for layer no 0
  shift_latt(2,0) = 0.0d0*A1          !y value for layer no 0

  shift_latt(1,1) = 0.5d0*A0          ! x value for layer no 1
  shift_latt(2,1) = 1.0d0/3.0d0 * A1  ! y value for layer no 1
 
  shift_latt(1,2) = 0.0d0*A0          !x value for layer no 3
  shift_latt(2,2) = 2.0d0/3.0d0*A1    !y value for layer no3
...
!----    arrange MA molecules (for (111) surface)
     do iz=1,zmaxma(imatom)
        do iy=1,ymaxma(imatom) / 4
          do ix=1,xmaxma(imatom) / 2

              do i = 1, 8  ! loop for basic lattice
                 icount = 0
                 imole = imole + 1
                 ifsetcor(imole) = .true.
...
90               continue
                 ...
                 if (icrema == 1) then ! first layer
                  
                    snx = DBLE(ix-1) * A0 * 2.0d0 &
                         & + base_latt(1,i) &
                         & + shift_latt(1,1)

                    sny = DBLE(iy-1) * A1 * 4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,1)

                    snz = DBLE(iz-1) * A2 + vacum_layer

                 else if (icrema == 2) then ! bottom layer(temp control)
                    ...
                    snz = DBLE(iz-1) * A2 + first_layer

                 else if (icrema == 3) then ! bottom layer
                    ...
                    snz = DBLE(iz-1) * A2 + second_layer

                 else if (icrema == 4) then ! upper layer
                    ...
                    sny = DBLE(iy-1) * A1 * 4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,mod(iz+1,3))
                    snz = DBLE(iz-1) * A2 + upper_liquid

                   else if (icrema == 5) then ! upper layer(temp control)
                    ...
                    sny = DBLE(iy-1) * A1 * 4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,2)
                    snz = DBLE(iz-1) * A2 + third_layer

                 else if (icrema == 6) then ! upper layer
                    ...
                    sny = DBLE(iy-1) * A1 * 4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,0)
                    snz = DBLE(iz-1) * A2 + last_layer

                 else if (icrema == 7) then ! ar1

                    snx = genrand_res53()*xcel
                    sny = genrand_res53()*ycel
                    snz = lower_liquid + genrand_res53()*liquid_layer

                 else if (icrema == 8) then ! ar1

                    snx = genrand_res53()*xcel
                    sny = genrand_res53()*ycel
                    snz = lower_liquid + genrand_res53()*liquid_layer

                 !else if (icrema == 9) then ! ar2

                  !  snx = genrand_res53()*xcel
                  !  sny = genrand_res53()*ycel
                  !  snz = lower_liquid + liquid_adsorb + genrand_res53()*liquid_layer
...
110           rxp = genrand_res53()*xcel
              ryp = genrand_res53()*ycel
              rzp = lower_liquid + genrand_res53()*liquid_layer
...
100        psi =   (genrand_res53() - 0.5d0) * 2.0d0 * pi
           ...
           rot_m(1,1) =   dcos(psi)*dcos(phi) - dcos(theta)*dsin(phi)*dsin(psi)
           rot_m(1,2) =   dcos(psi)*dsin(phi) + dcos(theta)*dcos(phi)*dsin(psi)
           rot_m(1,3) =   dsin(psi)*dsin(theta)
           rot_m(2,1) = - dsin(psi)*dcos(phi) - dcos(theta)*dsin(phi)*dcos(psi)
           rot_m(2,2) = - dsin(psi)*dsin(phi) + dcos(theta)*dcos(phi)*dcos(psi)
           ...
           do j= j1, j2
              ...
              atmcor(1,k) =  rot_m(1,1)*atmcor_t(1) + rot_m(2,1)*atmcor_t(2) + rot_m(3,1)*atmcor_t(3) + rxw
              atmcor(2,k) =  rot_m(1,2)*atmcor_t(1) + rot_m(2,2)*atmcor_t(2) + rot_m(3,2)*atmcor_t(3) + ryw
              atmcor(3,k) =  rot_m(1,3)*atmcor_t(1) + rot_m(2,3)*atmcor_t(2) + rot_m(3,3)*atmcor_t(3) + rzw
...
!---- check the atom is very close to the other atoms
              do m = 1, nmole
                 ...
                 do n = n1, n2
                    ...
                    r_ij(1:3) = atmcor(1:3,k) - atmcor(1,nn)
                    r_ij(1) = r_ij(1) - xcel * dnint(r_ij(1)/xcel)
                    r_ij(2) = r_ij(2) - ycel * dnint(r_ij(2)/ycel)
                    r_ij(3) = r_ij(3) - zcel * dnint(r_ij(3)/zcel)
