...
!---- valiables for ewald chart
!      real*8:: pot(0:ndiv,0:ndiv,0:nzdiv) ! potential for ewald-chart
!      real*8:: fxc(0:ndiv,0:ndiv,0:nzdiv) ! x-force for ewald-chart
!      real*8:: fyc(0:ndiv,0:ndiv,0:nzdiv) ! y-force for ewald-chart
!      real*8:: fzc(0:ndiv,0:ndiv,0:nzdiv) ! z-force for ewald-chart
...
!     temporary temp.
  ...
  real(8):: pext_c           ! external pressure in NPT(MTK)

  real(8):: expcdt2 = 0.0d0  ! = exp(-dt/2*(vxi(1)+(1+d/Nf)*veps))
  real(8):: expcdt4 = 0.0d0  ! = exp(-dt/4*(vxi(1)+(1+d/Nf)*veps))

  real(8):: e2               ! = 1/3!
  real(8):: e4               ! = 1/5!
  real(8):: e6               ! = 1/7!
  real(8):: e8               ! = 1/9!
  real(8):: aa               ! = exp(dt_short/2*veps)
  real(8):: aa2              ! = exp(dt_short*veps)
  real(8):: arg2             ! = (dt_short/2*veps)**2
  real(8):: poly             ! = sinh(dt_short/2*veps)/(dt_short/2*veps)
  real(8):: bb               ! = poly*dt_short*exp(dt_short/2*veps)
  real(8):: rv               ! = poly*exp(dt_short/2*veps)

  real(8):: vext             ! = vxi(1)+(1+d/Nf)*veps
  real(8):: argv2            ! = (dt_long/4*vext)^2
  real(8):: polyv            ! = sinh(dt_long/4*vext)/(dt_long/4*vext)
  real(8):: bbv              ! = polyv*dt_long/2*exp(-dt_long/4*vext)
  real(8):: cf               ! = polyv*exp(-dt_long/4*vext)

  real(8):: expcdt2v(1:3) = 0.0d0  ! = exp(-dt/2*vextv(:))
  real(8):: expcdt4v(1:3) = 0.0d0  ! = exp(-dt/4*vextv(:))

  real(8):: vextv(1:3)       ! = vxi(1)+vboxg(:)+1/Nf*Tr(vboxg(:))
  real(8):: argv2v(1:3)      ! = (dt_long/4*vextv(:))^2
  real(8):: polyvv(1:3)      ! = sinh(dt_long/4*vextv(;))/(dt_long/4*vextv(:))
  real(8):: bbvv(1:3)        ! = polyvv(:)*dt_long/2*exp(-dt_long/4*vextv(:))
  real(8):: cfv(1:3)         ! = polyvv(:)*exp(-dt_long/4*vextv(:))

  real(8):: aav(1:3)         ! = exp(dt_short/2*vboxg(:))
  real(8):: aa2v(1:3)        ! = exp(dt_short*vboxg(:))
  real(8):: arg2v(1:3)       ! = (dt_short/2*vboxg(:))**2
  real(8):: polyrv(1:3)      ! = sinh(dt_short/2*vboxg(:))/(dt_short/2*vboxg(:))
  real(8):: bbrv(1:3)        ! = polyrv(:)*dt_short*exp(dt_short/2*vboxg(:))
  real(8):: rvv(1:3)         ! = aav(:) * polyrv(:)

  integer:: pcont_dir        ! direction for uniaxial NPT control
                             ! (= 0 full anisotropic NPT)
...
!----- some preparation -----
  ...
  pcont_dir = 0   ! fully anisotropic NPT
  if (pcont_axis == 'x') then
      pcont_dir = MTK_XAXIS
  else if (pcont_axis == 'y') then
      pcont_dir = MTK_YAXIS
  else if (pcont_axis == 'z') THEN
      pcont_dir = MTK_ZAXIS
  end if
...
!!! Time measure
        ...
        ! isotropic NPT control
        if (pcont_axis == 'iso') then
            call nhcpisoint(degfree_all, &
                &           dt_long_cal, &
                &           mchain,text_c, &
                &           next,nyosh, &
                &           pint,pext_c)

        ! anisotropic NPT control
        else
            call nhcpanisoint(degfree_all, &
                &             dt_long_cal, &
                &             mchain,text_c, &
                &             next,nyosh, &
...
!             calculate coefficient of velocity(for_long) update
              if (pcont_axis == 'iso') then   ! isotropic NPT
                 vext = vlogs(1) &
                    & + (1.0d0+3.0d0/dble(degfree_all))*vlogv
                                ! = vxi(1)+(1+d/Nf)*veps
                 expcdt4 = exp(-0.25d0*dt_long_cal*vext)
                                ! = exp(-dt/4*(vxi(1)+(1+d/Nf)*veps))
                 expcdt2 = expcdt4 * expcdt4
                                ! = exp(-dt/2*(vxi(1)+(1+d/Nf)*veps))
                 argv2 = (0.25d0*dt_long_cal*vext)**2
                                ! = (dt_long/4*vext)^2
                 polyv = 1.0d0 &
                     & + argv2*(e2 + argv2 &
                     &  *(e4 + argv2*(e6 + argv2*e8)))
                                ! = sinh(dt_long/4*vext)/(dt_long/4*vext)
                 cf = expcdt4 * polyv
                                ! = polyv*exp(-dt_long/4*vext)
                 bbv = cf * 0.5d0 * dt_long_cal
                                ! = polyv*dt_long/2*exp(-dt_long/4*vext)
                 ...
                 if (pcont_dir == 0) then   ! full anisotropic NPT
                     vextv(1:3) = vlogs(1) &
                     &          + vboxg(1:3) &
                     &          + 1.0d0/dble(degfree_all) &
                     &          * (vboxg(1)+vboxg(2)+vboxg(3))
                                ! = vxi(1)+vboxg(:)+1/Nf*Tr(vboxg(:))
                 else   ! uniaxial NPT
                     vextv(1:3) = vlogs(1)
                     vextv(pcont_dir) = vextv(pcont_dir) &
                     &          + vboxg(pcont_dir) &
                     &          + 1.0d0/dble(degfree_all) &
                     &          * vboxg(pcont_dir)
                              ! = vxi(1)+vboxg(pcont_dir)+1/Nf*vboxg(pcont_dir)
                 end if
                 expcdt4v(1:3) = exp(-0.25d0*dt_long_cal*vextv(1:3))
                                ! = exp(-dt/4*vextv(:))
                 expcdt2v(1:3) = expcdt4v(1:3) * expcdt4v(1:3)
                                ! = exp(-dt/2*vextv(:))
                 argv2v(1:3) = (0.25d0*dt_long_cal*vextv(1:3))**2
                                ! = (dt_long/4*vextv(:))^2
                 polyvv(1:3) = 1.0d0 &
                           & + argv2v(1:3)*(e2 + argv2v(1:3) &
                           &  *(e4 + argv2v(1:3)*(e6 + argv2v(1:3)*e8)))
                                ! = sinh(dt_long/4*vextv(:))/(dt_long/4*vext(:))
                 cfv(1:3) = expcdt4v(1:3) * polyvv(1:3)
                                ! = polyvv(:)*exp(-dt_long/4*vextv(:))
                 bbvv(1:3) = cfv(1:3) * 0.5d0 * dt_long_cal
                                ! = polyvv(:)*dt_long/2*exp(-dt_long/4*vextv(:))

                 do i = 1, natom
                    atmvel(1:3,i) = atmvel(1:3,i)*expcdt2v(1:3) &
                                & + for_long(1:3,i)*bbvv(1:3)
                                ! higher factorized version
                                ! vi' = vi*expcdt2v(:)
                                !      +Fi/mi*bbvv(:)
                 end do
...
!          -- UPDATE COORDINATE AND VOLUME(in NPT) --

!          isotropic NPT
           if (pcont_axis == 'iso') then
              aa  = exp(0.5d0*dt_short_cal*vlogv) ! = exp(dt_short/2*veps)
              aa2 = aa*aa      ! = exp(dt_short*veps)
              arg2 = (0.5d0*dt_short_cal*vlogv)**2  ! = (dt_short/2*veps)**2
              poly = 1.0d0 &
                 & + arg2*(e2 + arg2*(e4 + arg2*(e6 + arg2*e8)))
                                ! = sinh(dt_short/2*veps)/(dt_short/2*veps)
              bb = aa * poly * dt_short_cal
                                ! = poly*dt_short*exp(dt_short/2*veps)

              rv = aa * poly   ! = aa * poly

!             update coordinate
              atmcor(1:3,1:natom) = atmcor(1:3,1:natom)*aa2 &
              &                   + atmvel(1:3,1:natom)*bb

!             update volume
              xlogv = xlogv + vlogv*dt_short_cal
              call calvol(xref, current_step, &
              &           xcel, ycel, zcel, &
              &           yratio, zratio, &
              &           xlogv, &
              &           rcut_book, ifbook,rcut, &
...
!          anisotropic NPT
              ...
              aav(1:3)  = exp(0.5d0*dt_short_cal*vboxg(1:3))
                                ! = exp(dt_short/2*vboxg(:))
              aa2v(1:3) = aav(1:3)*aav(1:3)      ! = exp(dt_short*vboxg(:))
              arg2v(1:3) = (0.5d0*dt_short_cal*vboxg(1:3))**2
                                ! = (dt_short/2*vboxg(:))**2
              polyrv(1:3) = 1.0d0 &
              &           + arg2v(1:3)*(e2 + arg2v(1:3) &
              &                        *(e4 + arg2v(1:3)*(e6 + arg2v(1:3)*e8)))
                            ! = sinh(dt_short/2*vboxg(:))/(dt_short/2*vboxg(:))
              bbrv(1:3) = aav(1:3) * polyrv(1:3) * dt_short_cal
                                ! = polyrv(:)*dt_short*exp(dt_short/2*vboxg(:))

              rvv(1:3) = aav(1:3) * polyrv(1:3)   ! = aav(:) * polyrv(:)

!             update coordinate
              do i = 1, natom
                 atmcor(1:3,i) = atmcor(1:3,i)*aa2v(1:3) &
                 &             + atmvel(1:3,i)*bbrv(1:3)
              end do

!             update cell box
              xboxh(1:3) = xboxh(1:3)*exp(vboxg(1:3)*dt_short_cal)
              call calbox(xref, current_step, &
              &           xcel, ycel, zcel, &
              &           yratio, zratio, &
              &           xboxh, &
              &           rcut_book, ifbook,rcut, &
...
!          -- update Ewald information
           if (mts_ewk == MTS_SHORT) then ! update kwave
              if (ifewald) then
                 call upewk( alpha, &
                      &      xcel, ycel, zcel )
              end if
...
!!! Time measure
        ...
        END DO              ! end of loop over NPT-XO-RESPA

!           --- Calculate constraint force & virial (NVE and NVT) ---

!            if ((ifrattle) .and. (.not. ifdomtk)) then
!               if (ifcalpremole .or. ifcalpreatom) then
!
!                  if (ifdonhc) then
!                     call calconst_nhc(maxnatom,natom,
!     &                        atmcor,atmmass,atmvel,
!     &                        nconst,iconst,jconst,dconst,
!     &                        dt_long_cal,
!     &                        for_long,for_short,
!     &                        atm_viri_const,atm_virit_const,
!     &                        vlogs,
!     &                        nstep_vir,eps_rattle)
!                  else
!                     call calconst(maxnatom,natom,
!     &                    atmcor,atmmass,atmvel,
!     &                    nconst,iconst,jconst,dconst,
!     &                    dt_long_cal,
!     &                    for_long,for_short,
...
!            ene_tot = ene_kin_poly + ene_kin_water + ene_kin_ma + pot_tot
        ...
        ene_conserve = ene_conserve + extra_pot_ba
