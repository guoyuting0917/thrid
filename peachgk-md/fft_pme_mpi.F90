!**************************************
!*  fft_pme_mpi.f Ver.1.7 '10.06.28   *
!*      for peachgk_md.f              *
!*            by G.Kikugawa           *
!**************************************
...
!
!---- MPI version written by G. Kikugawa ----
!
...
!      do k=1,nfftdim3
  do k= zlimit1(irank), zlimit2(irank)
     Q(1:2,1:nfftdim1,1:nfftdim2,k) = 0.0d0
  end do
...
!!! PME MPI
        if((k < zLimit1(irank)) .or. (zLimit2(irank) < k)) then
           cycle
        endif
...
subroutine fft_back(array,   &
  ...
  use interface_interact, only: FFTdata_trans_xz
  ...
  integer:: dim1_s, dim3_s
  integer:: nxdiv

  real(8):: send_array(2,nfft2,zlimitmax,xlimitmax*nproc)
...
!
!  do dim3=1,nfft3
  do dim3 = zlimit1(irank), zlimit2(irank)
     do dim2 = 1, nfft2
        do dim1 = 1, nfft1
...
!     do dim3=1,nfft3
  do nxdiv = 0, nproc-1
     dim1_s = xlimitmax * nxdiv
     do dim1 = xlimit1(nxdiv), xlimit2(nxdiv)
        dim1_s = dim1_s + 1
        ...
        do dim3 = zlimit1(irank), zlimit2(irank)
           ...
           do dim2 = 1, nfft2
              send_array(1,dim2,dim3_s,dim1_s) = DBLE(fftWork1(dim2))
              send_array(2,dim2,dim3_s,dim1_s) = AIMAG(fftWork1(dim2))
...
! - transfer FFT data
  call FFTdata_trans_xz(send_array,array,   &
       &                nfft1,nfft2,nfft3,   &
       &                nfftdim1,nfftdim2,nfftdim3)
...
!
  do dim2=1,nfft2
!  do dim1=1,nfft1
     do dim1=xlimit1(irank),xlimit2(irank)
        do dim3=1,nfft3
...
! do 100 ind = 1,indtop-1
  xlen = xlimit2(irank) - xlimit1(irank) + 1
  nff = xlen*nfft2
  indtop = xlen*nfft2*nfft3
  do ind = start_ind, indtop - 1
...
!           ind = (k1-1) + (k2-1)*nfft1 + (k3-1)*nfft2*nfft1
     ...
     jnd = ind - (k3-1)*nff
     k2 = jnd/xlen + 1
     k1 = jnd - (k2-1)*xlen + xlimit1(irank)
     m1 = k1 - 1
...
!!! Now PME energy and virial are not fully parallelized
...
!
!      vir(1,1) = vir(1,1) - 0.5d0*vir_tmp(1,1)
!      vir(2,1) = vir(2,1) - 0.5d0*vir_tmp(2,1)
!      vir(3,1) = vir(3,1) - 0.5d0*vir_tmp(3,1)
!      vir(1,2) = vir(1,2) - 0.5d0*vir_tmp(1,2)
!      vir(2,2) = vir(2,2) - 0.5d0*vir_tmp(2,2)
!      vir(3,2) = vir(3,2) - 0.5d0*vir_tmp(3,2)
!      vir(1,3) = vir(1,3) - 0.5d0*vir_tmp(1,3)
!      vir(2,3) = vir(2,3) - 0.5d0*vir_tmp(2,3)
!      vir(3,3) = vir(3,3) - 0.5d0*vir_tmp(3,3)
...
subroutine fft_forward(array,   &
  ...
  use interface_interact, only: FFTdata_trans_zx
  ...
  real(8):: send_array(2,nfft2,xlimitmax,zlimitmax*nproc)
...
!     do dim3=1,nfft3
  do nzdiv = 0, nproc-1
     dim3_s = zlimitmax * nzdiv
     do dim3 = zlimit1(nzdiv), zlimit2(nzdiv)
        dim3_s = dim3_s + 1
        ...
        do dim1 = xlimit1(irank), xlimit2(irank)
           ...
           do dim2=1,nfft2
              send_array(1,dim2,dim1_s,dim3_s) = DBLE(fftWork1(dim2))
              send_array(2,dim2,dim1_s,dim3_s) = AIMAG(fftWork1(dim2))
           enddo
...
! - transfer FFT data
  call FFTdata_trans_zx(send_array,array,   &
       &                nfft1,nfft2,nfft3,   &
       &                nfftdim1,nfftdim2,nfftdim3)
...
!      real*8:: frc_lcl(3,natom/nproc+1) !local force array
!      real*8:: frc_gbl(3,natom)         !global force array
!
!      integer:: count_a(nproc),displ_a(nproc)
!                    ! local natom count and displacement of each process
!      integer:: count_a3(nproc),displ_a3(nproc)
!                    ! data transfer count and displacement of each process
!      integer:: natom_l,natom_ll,mod_a !these are tmp variables.
!      integer:: nn              ! new loop indices
...
! LOCAL:
  real(8):: recv_array(2,nfft2,xlimitmax*zlimitmax*nproc)

  integer:: dim1, dim2, dim3
  integer:: dim1_s, dim3_s
  integer:: dim2d, dim2d_tmp, dim2d_tmp2
  integer:: nzdiv

!     +     +     +     +     +     +     +     +     +

!      
! MPI FFT data transfer ALL to ALL
!

  call MPI_ALLTOALL(send_array,xlimitmax*zlimitmax*2*nfft2,   &
       &            MPI_DOUBLE_PRECISION,   &
       &            recv_array,zlimitmax*xlimitmax*2*nfft2,   &
       &            MPI_DOUBLE_PRECISION,   &
       &            MPI_COMM_WORLD,ierror)

!
! store receive data to charge grid array
!
  do nzdiv = 0, nproc-1
     dim2d_tmp2 = zlimitmax*xlimitmax * nzdiv

     dim1_s = 0
     do dim1 = xlimit1(irank), xlimit2(irank)
        dim2d_tmp = dim2d_tmp2 + zlimitmax*dim1_s
        dim1_s = dim1_s + 1

        dim3_s = 0
        do dim3 = zlimit1(nzdiv), zlimit2(nzdiv)
           dim3_s = dim3_s + 1
           dim2d = dim2d_tmp + dim3_s
               
           do dim2 = 1, nfft2

              array(1:2,dim1,dim2,dim3) = recv_array(1:2,dim2,dim2d)
...
! LOCAL:
  real(8):: recv_array(2,nfft2,zlimitmax*xlimitmax*nproc)
...
!
  ...
  call MPI_ALLTOALL(send_array,zlimitmax*xlimitmax*2*nfft2,   &
       &            MPI_DOUBLE_PRECISION,   &
       &            recv_array,xlimitmax*zlimitmax*2*nfft2,   &
       &            MPI_DOUBLE_PRECISION,   &
       &            MPI_COMM_WORLD,ierror)
...
!
  do nxdiv = 0, nproc-1
     dim2d_tmp2 = xlimitmax*zlimitmax * nxdiv

     dim3_s = 0
     do dim3 = zlimit1(irank), zlimit2(irank)
        dim2d_tmp = dim2d_tmp2 + xlimitmax*dim3_s
        dim3_s = dim3_s + 1

        dim1_s = 0
        do dim1 = xlimit1(nxdiv), xlimit2(nxdiv)
           dim1_s = dim1_s + 1
           dim2d = dim2d_tmp + dim1_s
