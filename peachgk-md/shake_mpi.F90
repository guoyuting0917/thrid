...
!               dij2 = dij*dij   ! dij**2
           ...
           atmcor_tmp(1:3,ib) = atmcor_tmp(1:3,ib) - amassi_inv * GRij(1:3)
           atmcor_tmp(1:3,jb) = atmcor_tmp(1:3,jb) + amassj_inv * GRij(1:3)
...
!           - RATTLE failed to converge, so quitting -
        ...
        write(6,999)           maxiter
999     format(/3x, &
             & 'ERROR: SHAKE(C) failed to converge within MAXITER = ', &
             & i3)
        STOP
     end if
