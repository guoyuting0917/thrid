...
!---- temperature control factor
  real(8):: tfactor_poly(maxnpolytyp)
                                ! temperature control factor of polymer1
  real(8):: tfactor_water    ! temperature control factor of H2O
  real(8):: tfactor_ma(maxnmatyp) ! temperature control factor of MA
...
!---- center of mass of each molecule
  real(8):: molecom(3,nmole) ! center of mass of molecule
...
!     potential for each atom
  real(8):: pot_bo_long_atm(natom)    ! bond potential of each atom
  real(8):: pot_bo_med_atm(natom)     ! bond potential of each atom
  real(8):: pot_bo_short_atm(natom)   ! bond potential of each atom
  real(8):: pot_an_long_atm(natom)    ! angle potential of each atom
  real(8):: pot_an_med_atm(natom)     ! angle potential of each atom
  real(8):: pot_an_short_atm(natom)   ! angle potential of each atom
  real(8):: pot_to_long_atm(natom)    ! torsion potential of each atom
  real(8):: pot_to_med_atm(natom)     ! torsion potential of each atom
  real(8):: pot_to_short_atm(natom)   ! torsion potential of each atom
  real(8):: pot_14_long_atm(natom)    ! 1-4 int potential of each atom
  real(8):: pot_14_med_atm(natom)     ! 1-4 int potential of each atom
  real(8):: pot_14_short_atm(natom)   ! 1-4 int potential of each atom
  real(8):: pot_elin_long_atm(natom)  ! elc (intra) int potential of each atom
  real(8):: pot_elin_med_atm(natom)   ! elc (intra) int potential of each atom
  real(8):: pot_elin_short_atm(natom) ! elc (intra) int potential of each atom
  real(8):: pot_ljin_long_atm(natom)  ! VDW (intra) int potential of each atom
  real(8):: pot_ljin_med_atm(natom)   ! VDW (intra) int potential of each atom
  real(8):: pot_ljin_short_atm(natom) ! VDW (intra) int potential of each atom

  real(8):: pot_elc_long_atm(natom)     ! Coulomb potential of each atom
  real(8):: pot_elc_med_atm(natom)      ! Coulomb potential of each atom
  real(8):: pot_elc_short_atm(natom)    ! Coulomb potential of each atom
  real(8):: pot_vdw_long_atm(natom)     ! VDW potential of each atom
  real(8):: pot_vdw_med_atm(natom)      ! VDW potential of each atom
  real(8):: pot_vdw_short_atm(natom)    ! VDW potential of each atom
  real(8):: pot_mor_long_atm(natom)     ! Morse potential of each atom
  real(8):: pot_mor_med_atm(natom)      ! Morse potential of each atom
  real(8):: pot_mor_short_atm(natom)    ! Morse potential of each atom
  real(8):: pot_sh_long_atm(natom)      ! SH potential of each atom
  real(8):: pot_sh_med_atm(natom)       ! SH potential of each atom
  real(8):: pot_sh_short_atm(natom)     ! SH potential of each atom
  real(8):: pot_rfh_long_atm(natom)     ! RFH potential of each atom
  real(8):: pot_rfh_med_atm(natom)      ! RFH potential of each atom
  real(8):: pot_rfh_short_atm(natom)    ! RFH potential of each atom
  real(8):: pot_dou_long_atm(natom)     ! DOU potential of each atom
  real(8):: pot_dou_med_atm(natom)      ! DOU potential of each atom
  real(8):: pot_dou_short_atm(natom)    ! DOU potential of each atom
  real(8):: pot_cstmnb_long_atm(natom)  ! custom NB potential of each atom
  real(8):: pot_cstmnb_med_atm(natom)   ! custom NB potential of each atom
  real(8):: pot_cstmnb_short_atm(natom) ! custom NB potential of each atom
  real(8),allocatable:: pot_cstmnbex_long_atm(:,:)
                                        ! custom NB potential of each atom
  real(8),allocatable:: pot_cstmnbex_med_atm(:,:)
                                        ! custom NB potential of each atom
  real(8),allocatable:: pot_cstmnbex_short_atm(:,:)
                                        ! custom NB potential of each atom

!     virial tensor term for each atom
  real(8):: viribot_long_atm(3,3,natom,nhfregion) ! long-range (bond)
  real(8):: viribot_med_atm(3,3,natom,nhfregion) ! medium-range (bond)
  real(8):: viribot_short_atm(3,3,natom,nhfregion) ! short-range (bond)
  real(8):: viriant_long_atm(3,3,natom,nhfregion) ! long-range (angle)
  real(8):: viriant_med_atm(3,3,natom,nhfregion) ! medium-range (angle)
  real(8):: viriant_short_atm(3,3,natom,nhfregion) ! short-range (angle)
  real(8):: viritot_long_atm(3,3,natom,nhfregion) ! long-range (torsion)
  real(8):: viritot_med_atm(3,3,natom,nhfregion) ! medium-range (torsion)
  real(8):: viritot_short_atm(3,3,natom,nhfregion) ! short-range (torsion)
  real(8):: viri14t_long_atm(3,3,natom,nhfregion) ! long-range (1-4)
  real(8):: viri14t_med_atm(3,3,natom,nhfregion) ! medium-range (1-4)
  real(8):: viri14t_short_atm(3,3,natom,nhfregion) ! short-range (1-4)
  real(8):: virielint_long_atm(3,3,natom,nhfregion) ! long-range (elc intra)
  real(8):: virielint_med_atm(3,3,natom,nhfregion) ! medium-range (elc intra)
  real(8):: virielint_short_atm(3,3,natom,nhfregion) ! short-range (elc intra)
  real(8):: viriljint_long_atm(3,3,natom,nhfregion) ! long-range (L-J intra)
  real(8):: viriljint_med_atm(3,3,natom,nhfregion) ! medium-range (L-J intra)
  real(8):: viriljint_short_atm(3,3,natom,nhfregion) ! short-range (L-J intra)

  real(8):: virielct_long_atm(3,3,natom,nhfregion) ! long-range (Coulomb)
  real(8):: virielct_med_atm(3,3,natom,nhfregion) ! medium-range (Coulomb)
  real(8):: virielct_short_atm(3,3,natom,nhfregion) ! short-range (Coulomb)
  real(8):: viriljt_long_atm(3,3,natom,nhfregion) ! long-range (L-J)
  real(8):: viriljt_med_atm(3,3,natom,nhfregion) ! medium-range (L-J)
  real(8):: viriljt_short_atm(3,3,natom,nhfregion) ! short-range (L-J)
  real(8):: virimort_long_atm(3,3,natom,nhfregion) ! long-range (Morse)
  real(8):: virimort_med_atm(3,3,natom,nhfregion) ! medium-range (Morse)
  real(8):: virimort_short_atm(3,3,natom,nhfregion) ! short-range (Morse)
  real(8):: virisht_long_atm(3,3,natom,nhfregion) ! long-range (SH)
  real(8):: virisht_med_atm(3,3,natom,nhfregion) ! medium-range (SH)
  real(8):: virisht_short_atm(3,3,natom,nhfregion) ! short-range (SH)
  real(8):: virirfht_long_atm(3,3,natom,nhfregion) ! long-range (RFH)
  real(8):: virirfht_med_atm(3,3,natom,nhfregion) ! medium-range (RFH)
  real(8):: virirfht_short_atm(3,3,natom,nhfregion) ! short-range (RFH)
  real(8):: viridout_long_atm(3,3,natom,nhfregion) ! long-range (DOU)
  real(8):: viridout_med_atm(3,3,natom,nhfregion) ! medium-range (DOU)
  real(8):: viridout_short_atm(3,3,natom,nhfregion) ! short-range (DOU)
  real(8):: viricstmnbt_long_atm(3,3,natom,nhfregion) ! long-range (custom NB)
  real(8):: viricstmnbt_med_atm(3,3,natom,nhfregion) ! medium-range (custom NB)
  real(8):: viricstmnbt_short_atm(3,3,natom,nhfregion) ! short-range (custom NB)
  real(8),allocatable:: viricstmnbext_long_atm(:,:,:,:,:)
                                                  ! long-range (custom NB)
  real(8),allocatable:: viricstmnbext_med_atm(:,:,:,:,:)
                                                  ! medium-range (custom NB)
  real(8),allocatable:: viricstmnbext_short_atm(:,:,:,:,:)
                                                  ! short-range (custom NB)

!     for heat flux sum
  real(8):: hfkin_atm(3,nhfregion) ! heat flux of kinetic part
  real(8),allocatable:: hfkinex_atm(:,:,:)
                              ! heat flux of kinetic part (extra custom NB)

  real(8):: hfpotbo_atm(3,nhfregion) ! heat flux of potential part (bond)
  real(8):: hfpotan_atm(3,nhfregion) ! heat flux of potential part (angle)
  real(8):: hfpotto_atm(3,nhfregion) ! heat flux of potential part (tors)
  real(8):: hfpot14_atm(3,nhfregion) ! heat flux of potential part (1-4)
  real(8):: hfpotelin_atm(3,nhfregion)
                                ! heat flux of potential part (elc intra)
  real(8):: hfpotljin_atm(3,nhfregion)
                                ! heat flux of potential part (L-J intra)

  real(8):: hfpotelc_atm(3,nhfregion) ! heat flux of potential part (Coulomb)
  real(8):: hfpotlj_atm(3,nhfregion) ! heat flux of potential part (L-J)
  real(8):: hfpotmor_atm(3,nhfregion)
                                ! heat flux of potential part (Morse)
  real(8):: hfpotsh_atm(3,nhfregion)
                                ! heat flux of potential part (SH)
  real(8):: hfpotrfh_atm(3,nhfregion)
                                ! heat flux of potential part (RFH)
  real(8):: hfpotdou_atm(3,nhfregion)
                                ! heat flux of potential part (DOU)
  real(8):: hfpotcstmnb_atm(3,nhfregion)
                                ! heat flux of potential part (custom NB)
  real(8),allocatable:: hfpotcstmnbex_atm(:,:,:)
                                ! heat flux of potential part (extra custom NB)

  real(8):: hfviribo_atm(3,nhfregion) ! heat flux of virial part (bond)
  real(8):: hfvirian_atm(3,nhfregion) ! heat flux of virial part (angle)
  real(8):: hfvirito_atm(3,nhfregion) ! heat flux of virial part (tors)
  real(8):: hfviri14_atm(3,nhfregion) ! heat flux of virial part (1-4)
  real(8):: hfvirielin_atm(3,nhfregion) ! heat flux of virial part (elc intra)
  real(8):: hfviriljin_atm(3,nhfregion) ! heat flux of virial part (L-J intra)

  real(8):: hfvirielc_atm(3,nhfregion) ! heat flux of virial part (Coulomb)
  real(8):: hfvirilj_atm(3,nhfregion) ! heat flux of virial part (L-J)
  real(8):: hfvirimor_atm(3,nhfregion)
                                ! heat flux of virial part (Morse)
  real(8):: hfvirish_atm(3,nhfregion)
                                ! heat flux of virial part (SH)
  real(8):: hfvirirfh_atm(3,nhfregion)
                                ! heat flux of virial part (RFH)
  real(8):: hfviridou_atm(3,nhfregion)
                                ! heat flux of virial part (DOU)
  real(8):: hfviricstmnb_atm(3,nhfregion)
                                ! heat flux of virial part (custom NB)
  real(8),allocatable:: hfviricstmnbex_atm(:,:,:)
                                ! heat flux of virial part (extra custom NB)

  real(8):: hftot_atm(3,nhfregion) ! total heat flux

!     for momentum flux sum
  real(8):: mfkin_atm(3,3,nhfregion) ! momentum flux of kinetic part
  real(8),allocatable:: mfkinex_atm(:,:,:,:)
                              ! momentum flux of kinetic part (extra custom NB)

  real(8):: mfviribo_atm(3,3,nhfregion) ! momentum flux of virial part (bond)
  real(8):: mfvirian_atm(3,3,nhfregion) ! momentum flux of virial part (angle)
  real(8):: mfvirito_atm(3,3,nhfregion) ! momentum flux of virial part (tors)
  real(8):: mfviri14_atm(3,3,nhfregion) ! momentum flux of virial part (1-4)
  real(8):: mfvirielin_atm(3,3,nhfregion)
                                ! momentum flux of virial part (elc intra)
  real(8):: mfviriljin_atm(3,3,nhfregion)
                                ! momentum flux of virial part (L-J intra)

  real(8):: mfvirielc_atm(3,3,nhfregion) ! momentum flux of virial part (Coulomb)
  real(8):: mfvirilj_atm(3,3,nhfregion) ! momentum flux of virial part (L-J)
  real(8):: mfvirimor_atm(3,3,nhfregion)
                                ! momentum flux of virial part (Morse)
  real(8):: mfvirish_atm(3,3,nhfregion)
                                ! momentum flux of virial part (SH)
  real(8):: mfvirirfh_atm(3,3,nhfregion)
                                ! momentum flux of virial part (RFH)
  real(8):: mfviridou_atm(3,3,nhfregion)
                                ! momentum flux of virial part (DOU)
  real(8):: mfviricstmnb_atm(3,3,nhfregion)
                                ! momentum flux of virial part (custom NB)
  real(8),allocatable:: mfviricstmnbex_atm(:,:,:,:)
                                ! momentum flux of virial part (extra custom NB)

  real(8):: mftot_atm(3,3,nhfregion) ! total momentum flux
...
!      real*8:: atmcor_att(3,maxnatom) ! atmcor at time T for const. force
  ...
  real(8):: atmcor_hf(3,natom) ! old atmcor for heat flux calculation
  real(8):: molecom_hf(3,nmole) ! center of mass of molecule (heat flux)

  real(8):: atmcor_mf(3,natom) ! old atmcor for momentum flux calculation
  real(8):: molecom_mf(3,nmole) ! center of mass of molecule (momentum flux)

  real(8):: dt_cross         ! interval time to check if cross the surface
                                !   only for surface-based method
                                ! = dt_long_cal * heatfinterval

  logical:: ifass_strmvel       ! if streaming velocity already assigned

!     temporary temp.
  real(8):: tcont_polyt(maxnpolytyp) ! poly Temp. [non-d] in NVT
  real(8):: tcont_watert     ! H2O Temp. [non-d] in NVT
  real(8):: tcont_mat(maxnmatyp) ! MA Temp. [non-d] in NVT

  real(8):: tcont_polyinit(maxnpolytyp) ! poly Temp. [non-d] in NVT
  real(8):: tcont_waterinit  ! H2O Temp. [non-d] in NVT
  real(8):: tcont_mainit(maxnmatyp) ! MA Temp. [non-d] in NVT
  ...
  real(8):: det_ene_kin(maxntcregion) ! for outputting thermal control data
...
!- CSTMNB function Ver. 2
 ...
 ncstmnbex = 3
  ...
  allocate(viricstmnbext_long_atm(3,3,natom,nhfregion,1:ncstmnbex))
  ! allocate(viricstmnbext_med_atm(3,3,natom,nhfregion,1:ncstmnbex))
  allocate(viricstmnbext_short_atm(3,3,natom,nhfregion,1:ncstmnbex))
  allocate(pot_cstmnbex_long_atm(natom,1:ncstmnbex))
  ! allocate(pot_cstmnbex_med_atm(natom,1:ncstmnbex))
  allocate(pot_cstmnbex_short_atm(natom,1:ncstmnbex))

  allocate(hfkinex_atm(3,nhfregion,1:ncstmnbex))
  allocate(hfpotcstmnbex_atm(3,nhfregion,1:ncstmnbex))
  allocate(hfviricstmnbex_atm(3,nhfregion,1:ncstmnbex))

  allocate(mfkinex_atm(3,3,nhfregion,1:ncstmnbex))
  allocate(mfviricstmnbex_atm(3,3,nhfregion,1:ncstmnbex))

!     ---- some preparations ----

  dt_cross = dt_long_cal * DBLE(heatfinterval)

!     --- TEMPERATURE CONTROL (V-SCALE) ---

!     - NVE ensemble
  tfactor_poly(1)  = 1.0d0
  tcont_polyt(1) = tcont_poly(1)
  tcont_polyinit(1) = tcont_poly(1)

  tfactor_poly(1:npolytyp)  = 1.0d0
  tcont_polyt(1:npolytyp) = tcont_poly(1:npolytyp)
  tcont_polyinit(1:npolytyp) = tcont_poly(1:npolytyp)

  tfactor_water = 1.0d0
  tcont_watert = tcont_water
  tcont_waterinit = tcont_water

  tfactor_ma(1:nmatyp) = 1.0d0
  tcont_mat(1:nmatyp) = tcont_ma(1:nmatyp)
  tcont_mainit(1:nmatyp) = tcont_ma(1:nmatyp)

  if (iflocalheat) then

     do i = 1, nlheat_poly
        tcont_polyt(index_nlheat_poly(i)) = tcont_nlheat_poly(i)
        tcont_polyinit(index_nlheat_poly(i)) = tcont_nlheat_poly(i)
     end do
     do i = 1, nlheat_water
        tcont_watert = tcont_nlheat_water
        tcont_waterinit = tcont_nlheat_water
     end do
     do i = 1, nlheat_ma
        tcont_mat(index_nlheat_ma(i)) = tcont_nlheat_ma(i)
        tcont_mainit(index_nlheat_ma(i)) = tcont_nlheat_ma(i)
     end do
...
!---- Impose Maxwell-Boltzmann distribution
     if (current_step == nstep_maxwell) then
        call mkmaxvel(npoly,nwater,nmatom, &
...
!---- NVE-MTS (velocity-Verlet algorism)
...
!       -- region Langevin thermostat
        if (ifreglange) then

           ! atom registration for region based Langevin thermostat
           call langevin_regist(npoly,npolytyp, &
                &               npoly_mole,npoly_atom, &
                   &               nwater, &
                   &               nmatom,nmatyp,nmatomtyp, &
                   &               degfree_poly,degfree_water, &
...
!           - calculate initial force & energy -
        ...
        call calforce_hf(MTS_SHORT,for_short, &
             ...
             &           pot_to_short_atm, &
             &           pot_14_short_atm, &
             &           pot_elin_short_atm,pot_ljin_short_atm, &
             &           pot_elc_short_atm, &
             &           pot_vdw_short_atm, &
             &           pot_mor_short_atm, &
             &           pot_sh_short_atm, &
             ...
             &           viritot_short_atm, &
             &           viri14t_short_atm, &
             &           virielint_short_atm,viriljint_short_atm, &
             &           virielct_short_atm, &
             &           viriljt_short_atm, &
             &           virimort_short_atm, &
             &           virisht_short_atm, &
        ...
        call calforce_hf(MTS_LONG,for_long, &
             ...
             &           pot_to_long_atm, &
             &           pot_14_long_atm, &
             &           pot_elin_long_atm,pot_ljin_long_atm, &
             &           pot_elc_long_atm, &
             &           pot_vdw_long_atm, &
             &           pot_mor_long_atm, &
             &           pot_sh_long_atm, &
             ...
             &           pot_dou_long_atm, &
             &           pot_cstmnb_long_atm, &
             &           pot_cstmnbex_long_atm, &
             &           viribot_long_atm, &
             &           viriant_long_atm, &
             &           viritot_long_atm, &
             &           viri14t_long_atm, &
             &           virielint_long_atm,viriljint_long_atm, &
             &           virielct_long_atm, &
             &           viriljt_long_atm, &
             &           virimort_long_atm, &
             &           virisht_long_atm, &
             &           virirfht_long_atm, &
             &           viridout_long_atm, &
             &           viricstmnbt_long_atm, &
             &           viricstmnbext_long_atm, &
             &           mts_bond, mts_angl, mts_anglub, &
             &           mts_tors,mts_torsrb,mts_torsim, &
             &           mts_vdw,mts_ewr,mts_ewk, &
             &           mts_vdw14,mts_elc14, &
...
!       - initialization for outputting thermal control data
        if (ifoutthc) then
           det_ene_kin(1:maxntcregion) = 0.0d0
        end if
...
!       - region Langevin thermostat
        if (ifreglange) then
           ...
           call langevin_region(dt_long_cal, &
                &               iftcratom, &
                &               nlangeregion, &
                &               r_ltemp, r_ltdamp, &
                &               ifoutthc, &
...
!!! Time measure
           ...
           if (istep_short == 1) then
              atmvel(1:3,1:natom) =  atmvel(1:3,1:natom) &
                   &               + 0.5d0*dt_long_cal*for_long(1:3,1:natom)
           end if
...
!---- calculate center of mass of each molecule (for heat flux calculation)

           if (istep_short == nstep_short) then
                                           ! only when heat flux calculation
...
!!! Time measure
        ...
        END DO              ! end of loop over NVE-RESPA
...
!           --- Woodcock temperature control ---
!            if (md_cont .ne. md_nve) then
!               if ((mod(current_step,tcontinterval) .eq. 1) .or.
!     &            (tcontinterval .eq. 1) .or. (md_cont .eq. md_0k)) then
!
!                  if (.not. iflocalheat) then
!                     call contemp(npoly,npolytyp,npoly_mole,
!     &                   nwater,
!     &                   nmatom,nmatyp,nmatomtyp,
!     &                   degfree_poly,degfree_water,
!     &                   degfree_ma,degfree_all,
...
!                  else
!                     call contemp_e(npoly,npolytyp,npoly_mole,
!     &                     nwater,
!     &                     nmatom,nmatyp,nmatomtyp,
!     &                     degfree_poly,degfree_water,
!     &                     degfree_ma,degfree_all,
...
!
!               end if
!            else if ((md_cont .eq. md_nve) .and. iflocalheat) then
        if (iflocalheat) then
           if ((mod(current_step,tcontinterval) == 1) .or. &
                &            (tcontinterval == 1)) then
...
!             -- assign streaming velocity
              ...
              call contemp_nve( npoly,npolytyp,npoly_mole,npoly_atom, &
                   &            nwater, &
                   &            nmatom,nmatyp,nmatomtyp, &
                   &            degfree_poly,degfree_water, &
                   &            degfree_ma,degfree_all, &
...
!             -- assign streaming velocity
              ...
              call contemp_region(npoly,npolytyp, &
                   &              npoly_mole,npoly_atom, &
                   &              nwater, &
                   &              nmatom,nmatyp,nmatomtyp, &
                   &              degfree_poly,degfree_water, &
...
!             -- assign streaming velocity
              ...
              call conhf_region( npoly,npolytyp, &
                   &             npoly_mole,npoly_atom, &
                   &             nwater, &
                   &             nmatom,nmatyp,nmatomtyp, &
                   &             xcel,ycel,zcel, &
...
!!! Time measure
#if defined(TIME_M)
        call get_timecount_barrier( heatf_time, heatf_time_elps, 'START' )
#endif
!!! Time measure

!       heat flux calculation
!!!     When surface-based, always calculate transport terms
        if (.not. ifhfvol .or. &
        &   (mod(current_step,heatfinterval) == 1) .or. &
...
!             -- assign streaming velocity
              ...
              call calheatf(npoly,nwater,nmatom, &
                   &        xcel,ycel,zcel, &
                   &        ifhfvol, &
                   &        nhfregion,hfzpos1,hfzpos2, &
                   &        hftyp_atm, &
...
!       momentum flux calculation
           if(ifcalmtf) then
              ...
              call calmomentumf(npoly,nwater,nmatom, &
                   &            xcel,ycel,zcel, &
                   &            ifhfvol, &
                   &            nhfregion,hfzpos1,hfzpos2, &
                   &            hftyp_atm, &
...
!!! Time measure
#if defined(TIME_M)
        call get_timecount( heatf_time, heatf_time_elps, 'STOP' )
#endif
!!! Time measure
...
!           --- Output energy, trajectory, velocity etc. ---
        ...
        if (irank == 0) then
           ...
           if ((mod(current_step,outinterval) == 1) .or. &
              ...
              if (ifoutfor) then
                 call outfor_hf(oufor,current_step,fref, &
                 &              for_long,for_short, &
                 &              ifhfvol, &
                 &              nhfregion,hfzpos1,hfzpos2, &
                 &              hftyp_atm, &
           ...
           if (ifoutthc) then
              if ((mod(current_step,tcontinterval) == 1) .or. &
                 ...
                 if (ifregionheat) then   ! region temperature control
                    call outthc(outhc,current_step, &
                         &      xref,eref,timeref, &
                         &      ntcregion, &      ! ntcregion
                         &      det_ene_kin)

                 else if (ifreglange) then   ! region Langevin thermostat
                    call outthc(outhc,current_step, &
                         &      xref,eref,timeref, &
                         &      nlangeregion, &   ! nlangeregion
                         &      det_ene_kin)
           ...
           if ((mod(current_step,heatfinterval) == 1) .or. &
              ...
              if (ifcalhtf) then
                 ...
                 call outhtf(ouhtf,current_step, &
                      &      xref,eref,timeref, &
                      &      ifhfvol, &
                      &      nhfregion,hfzpos1,hfzpos2, &
                      &      hfkin_atm,hfkinex_atm, &
                 ...
                 ! clear cumulated transport terms (for surface-based)
                 if (.not. ifhfvol) then
...
!---- release memory
  ...
  deallocate(viricstmnbext_long_atm)
  ! deallocate(viricstmnbext_med_atm)
  deallocate(viricstmnbext_short_atm)
  deallocate(pot_cstmnbex_long_atm)
  ! deallocate(pot_cstmnbex_med_atm)
  deallocate(pot_cstmnbex_short_atm)

  deallocate(hfkinex_atm)
  deallocate(hfpotcstmnbex_atm)
  deallocate(hfviricstmnbex_atm)

  deallocate(mfkinex_atm)
  deallocate(mfviricstmnbex_atm)
