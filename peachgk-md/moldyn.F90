...
! --- TEMPERATURE CONTROL (V-SCALE) ---
  ...
  if (md_cont == md_0k) then ! NVT0K-1

     tfactor_poly(1) = 1.0d0
     tcont_polyt(1) = 0.0d0
     tcont_polyinit(1) = 0.0d0

     tfactor_poly(1:npolytyp) = 1.0d0
     tcont_polyt(1:npolytyp) = 0.0d0
     tcont_polyinit(1:npolytyp) = 0.0d0

     tfactor_water = 1.0d0
     tcont_watert = 0.0d0
     tcont_waterinit = 0.0d0

     tfactor_ma(1:nmatyp) = 1.0d0
     tcont_mat(1:nmatyp) = 0.0d0
     tcont_mainit(1:nmatyp) = 0.0d0

  else if (md_cont == md_h) then ! Heating

     tcont_polyt(1) = tcont_poly(1)
     tcont_polyinit(1) = tcont_poly_ini

     tcont_polyt(1:npolytyp) = tcont_poly(1:npolytyp)
     tcont_polyinit(1:npolytyp) = tcont_poly_ini

     tcont_watert = tcont_water
     tcont_waterinit = tcont_water_ini

     tcont_mat(1:nmatyp) = tcont_ma(1:nmatyp)
     tcont_mainit(1:nmatyp) = tcont_ma_ini

  else if (md_cont == md_t) then ! NVT
  ...
  else if (md_cont == md_nve) then ! NVE
...
!        --- TEMPERATURE CONTROL (V-SCALE) ---
     if (md_cont == md_h) then ! Heating
        tfactor_poly(1)  =  1.0d0 * dble(current_step - inistep) &
             &            / dble(endstep - inistep)
        do i = 1, npolytyp
           tfactor_poly(i)  =  1.0d0 * dble(current_step - inistep) &
                &            / dble(endstep - inistep)
           if (tfactor_poly(i) > 1.0d0) tfactor_poly(i) = 1.0d0
        end do
        tfactor_water =  1.0d0 * dble(current_step - inistep) &
             &         / dble(endstep - inistep)
        if (tfactor_water > 1.0d0) tfactor_water = 1.0d0
        do i = 1, nmatyp
           tfactor_ma(i) =  1.0d0 * dble(current_step - inistep) &
                &         / dble(endstep - inistep)
           if (tfactor_ma(i) > 1.0d0) tfactor_ma(i) = 1.0d0
        end do
...
!           for local heating
        if (iflocalheat) then
           ...
           do i = 1, nlheat_poly
              tfactor_poly(index_nlheat_poly(i)) = 1.0d0
           end do
           ...
           do i = 1, nlheat_ma
              tfactor_ma(index_nlheat_ma(i)) =  1.0d0
           end do
...
!           --- Woodcock temperature control ---
        if (md_cont /= md_nve) then
           if ((mod(current_step,tcontinterval) == 1) .or. &
                & (tcontinterval == 1) .or. (md_cont == md_0k)) then
...
!           - NVE ensemble (specific temp. control)
        else
