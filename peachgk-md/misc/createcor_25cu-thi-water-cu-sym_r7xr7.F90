!*************************************************
!*  createcor.f Ver.2.5cu-thi-water-cu '12.09.24 *
!*      for peachgk_md.f                         *
!*            by G.Kikugawa                      *
!**************************************************
...
!     parameters for fcc lattice
  ...
  real(8):: base_latt(2,98)
  real(8):: shift_latt(2,0:2)

  real(8):: base_latt_S1(2,14)
  real(8):: base_latt_S2(2,14)
...
!---- create base lattice for Cu (0 layer)
  ...
  base_latt(1,4) = 3.0d0*A0
  base_latt(2,4) = 0.0d0*A1
  base_latt(1,5) = 4.0d0*A0
  base_latt(2,5) = 0.0d0*A1
  base_latt(1,6) = 5.0d0*A0
  base_latt(2,6) = 0.0d0*A1
  base_latt(1,7) = 6.0d0*A0
  base_latt(2,7) = 0.0d0*A1
  ...
  do iy=2,14
     do ix=1,7
        ...
        base_latt(2,icount) = dble(iy-1)*A1
     end do
  end do
...
!---- create base lattice for thiol S (2 layer over 1 layer)
  ...
  base_latt_S1(1,2) = 3.0d0 * A0
  base_latt_S1(2,2) = (1.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,3) = 5.5d0 * A0
  base_latt_S1(2,3) = (2.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,4) = 1.0d0 * A0
  base_latt_S1(2,4) = (3.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,5) = 3.5d0 * A0
  base_latt_S1(2,5) = (4.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,6) = 6.0d0 * A0
  base_latt_S1(2,6) = (5.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,7) = 1.5d0 * A0
  base_latt_S1(2,7) = (6.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,8) = 4.0d0 * A0
  base_latt_S1(2,8) = (7.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,9) = 6.5d0 * A0
  base_latt_S1(2,9) = (8.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,10) = 2.0d0 * A0
  base_latt_S1(2,10) = (9.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,11) = 4.5d0 * A0
  base_latt_S1(2,11) = (10.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,12) = 0.0d0 * A0
  base_latt_S1(2,12) = (11.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,13) = 2.5d0 * A0
  base_latt_S1(2,13) = (12.0d0+1.0d0/3.0d0) * A1
  base_latt_S1(1,14) = 5.0d0 * A0
  base_latt_S1(2,14) = (13.0d0+1.0d0/3.0d0) * A1

  ! copy to base_latt_S2
  base_latt_S2(1:2,1:14) = base_latt_S1(1:2,1:14)

  ! x,y shift
  x_shift = 0.0d0 * A0
  y_shift = 2.0d0/3.0d0 * A1

  base_latt_S1(1,1:14) = base_latt_S1(1,1:14) + x_shift
  base_latt_S1(2,1:14) = base_latt_S1(2,1:14) + y_shift

!---- create base lattice for thiol S (0 layer over 1 layer)
  x_shift = -0.5d0 * A0
  y_shift = 1.0d0/3.0d0 * A1

  base_latt_S2(1,1:14) = base_latt_S2(1,1:14) + x_shift
  base_latt_S2(2,1:14) = base_latt_S2(2,1:14) + y_shift
...
!----    arrange MA molecules (for (111) surface)
     do iz=1,zmaxma(imatom)
        do iy=1,ymaxma(imatom) / 14
           do ix=1,xmaxma(imatom) / 7

              do i = 1, 98  ! loop for basic lattice
                 icount = 0
                 imole = imole + 1
                 ifsetcor(imole) = .true.
...
90               continue
                 ...
                 if (icrema == 1) then ! lower layer
                    ...
                    snx = DBLE(ix-1) * A0*7.0d0 &
                         & + base_latt(1,i) &
                         & + shift_latt(1,mod(iz-1,3))
                    sny = DBLE(iy-1) * A1*14.0d0 &
                         & + base_latt(2,i) &
...
!                    --- make gravity coordinate ---
                 ...
                 if (ipoly == 1)  then ! lower thiol
                    rxp = DBLE(ix-1) * A0*7.0d0 &
                         & + base_latt_S1(1,i)
                    ryp = DBLE(iy-1) * A1*14.0d0 &
                         & + base_latt_S1(2,i)
                    rzp = lower_S
