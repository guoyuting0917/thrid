!**************************************
!*  cell_expand.f90 Ver.1.4 '11.02.07 *
!*      for peachgk_md.f              *
!*            by G.Kikugawa           *
!**************************************
...
! LOCAL:
  ...
  real(8):: coeff_exp(3)     ! = r_expand^(1/3)
...
!  zcel = zratio*xcel
  ...
  coeff_exp(1) = 1.d0
  coeff_exp(2) = 1.d0
  coeff_exp(3) = r_expand

  xcel = xcel*coeff_exp(1)
  ycel = ycel*coeff_exp(2)
  zcel = zcel*coeff_exp(3)
...
!        - find the first (j1) & last (j2) atoms  of mol(i) -
     ...
     do j=j1,j2
        atmcor(1:3,j) = atmcor(1:3,j) * coeff_exp(1:3)
     end do
...
!     molgcor(1:3) = molgcor(1:3) * coeff_expand
     molgcor(1:3) = molgcor(1:3) * coeff_exp(1:3)
...
!        atmcor(1:3,k) = atmcor(1:3,k) * coeff_expand
     atmcor(1:3,k) = atmcor(1:3,k) * coeff_exp(1:3)
