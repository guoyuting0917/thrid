!*************************************************
!*  createcor.f Ver.2.5cu-thi-water-cu '12.11.20 *
!*      for peachgk_md.f                         *
!*            by G.Kikugawa                      *
!**************************************************
...
!---- notation ----
  ...
  s_scale = dble(nunit)/dble(nunit_org)

  ! memory allocation
  allocate(base_latt(2,nunit*nunit))
  allocate(base_latt_S1(2,4*nunit_org/4*nunit_org/4))
  allocate(base_latt_S2(2,4*nunit_org/4*nunit_org/4))
...
!----    arrange MA molecules (for (111) surface)
     do iz=1,zmaxma(imatom)
        do iy=1,ymaxma(imatom) / nunit
           do ix=1,xmaxma(imatom) / nunit

              do i = 1, nunit*nunit  ! loop for basic lattice
                 icount = 0
                 imole = imole + 1
                 ifsetcor(imole) = .true.
...
90               continue
                 ...
                 if (icrema == 1) then ! lower layer
                    ...
                    snx = DBLE(ix-1) * A0*dble(nunit) &
                         & + base_latt(1,i) &
                         & + shift_latt(1,mod(iz-1,3))
                    sny = DBLE(iy-1) * A1*dble(nunit) &
                         & + base_latt(2,i) &
...
!-------- Locate molecules of poly (for arranging thiol) --------
  ...
  do icrepo = 1, ncrecorpo
     ...
     if ((ipoly == 1) .or. (ipoly == 2)) then
        ymaxpo(ipoly) = ymaxpo(ipoly) / (nunit_org/2)
        xmaxpo(ipoly) = xmaxpo(ipoly) / (nunit_org/2)
!        baseloop = baseloop
     else
        baseloop = 1
     end if
...
!                    --- make gravity coordinate ---
                 ...
                 if (ipoly == 1)  then ! lower thiol
                    rxp = DBLE(ix-1) * A0*dble(nunit) &
                         & + base_latt_S1(1,i)
                    ryp = DBLE(iy-1) * A1*dble(nunit) &
                         & + base_latt_S1(2,i)
                    rzp = lower_S
