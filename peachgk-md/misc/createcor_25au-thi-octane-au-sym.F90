!*************************************************
!*  createcor_.f Ver.2.5au-thi-oct-au '07.04.07  *
!*      for peachgk_md.f                         *
!*            by G.Kikugawa                      *
!**************************************************
...
!     parameters for fcc lattice
  ...
  real(8):: base_latt_S1(2,4)
  real(8):: base_latt_S2(2,4)
...
!---- notation ----
  ...
  vacume_layer = 20.0d-10 / xref
  oct_layer = 50.0d-10 / xref
  thiol_layer = 10.0d-10 / xref

  lower_S = vacume_layer + DBLE(zmaxma(2)) * A2 + A3
  upper_S = lower_S + thiol_layer*2.0d0 + oct_layer

  lower_oct_z = lower_S + thiol_layer + 5.0d0
  oct_layer_z = oct_layer - 2.0d0*5.0d0

  upper_AU_z = upper_S + A3

!---- create base lattice for Au (1 layer)
  base_latt(1,1) = 0.0d0*A0
  base_latt(2,1) = 0.0d0*A1
  base_latt(1,2) = 1.0d0*A0
  base_latt(2,2) = 0.0d0*A1
  base_latt(1,3) = 2.0d0*A0
  base_latt(2,3) = 0.0d0*A1

  base_latt(1,4) = 0.5d0*A0
  base_latt(2,4) = 1.0d0*A1
  base_latt(1,5) = 1.5d0*A0
  base_latt(2,5) = 1.0d0*A1
  base_latt(1,6) = 2.5d0*A0
  base_latt(2,6) = 1.0d0*A1

  base_latt(1,7) = 0.0d0*A0
  base_latt(2,7) = 2.0d0*A1
  base_latt(1,8) = 1.0d0*A0
  base_latt(2,8) = 2.0d0*A1
  base_latt(1,9) = 2.0d0*A0
  base_latt(2,9) = 2.0d0*A1

  base_latt(1,10) = 0.5d0*A0
  base_latt(2,10) = 3.0d0*A1
  base_latt(1,11) = 1.5d0*A0
  base_latt(2,11) = 3.0d0*A1
  base_latt(1,12) = 2.5d0*A0
  base_latt(2,12) = 3.0d0*A1
...
!---- create base lattice for thiol S (0 layer over 2 layer)
  base_latt_S1(1,1) = 1.5d0 * A0
  base_latt_S1(2,1) = (0.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,2) = 0.0d0 * A0
  base_latt_S1(2,2) = (1.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,3) = 1.5d0 * A0
  base_latt_S1(2,3) = (2.0d0+1.0d0/3.0d0) * A1

  base_latt_S1(1,4) = 0.0d0 * A0
  base_latt_S1(2,4) = (3.0d0+1.0d0/3.0d0) * A1

!---- create base lattice for thiol S (0 layer over 1 layer)
  base_latt_S2(1,1) = 2.0d0 * A0
  base_latt_S2(2,1) = (0.0d0+2.0d0/3.0d0) * A1

  base_latt_S2(1,2) = 0.5d0 * A0
  base_latt_S2(2,2) = (1.0d0+2.0d0/3.0d0) * A1

  base_latt_S2(1,3) = 2.0d0 * A0
  base_latt_S2(2,3) = (2.0d0+2.0d0/3.0d0) * A1

  base_latt_S2(1,4) = 0.5d0 * A0
  base_latt_S2(2,4) = (3.0d0+2.0d0/3.0d0) * A1
...
!----    arrange MA molecules (for (111) surface)
     do iz=1,zmaxma(imatom)
        do iy=1,ymaxma(imatom) / 4
           do ix=1,xmaxma(imatom) / 3

              do i = 1, 12  ! loop for basic lattice
                 icount = 0
                 imole = imole + 1
                 ifsetcor(imole) = .true.
...
90               continue
                 ...
                 if (icrema == 1) then ! first fix layer
                  
                    snx = DBLE(ix-1) * A0*3.0d0 &
                         & + base_latt(1,i) &
                         & + shift_latt(1,mod(iz-1,3))
                    sny = DBLE(iy-1) * A1*4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,mod(iz-1,3))
                    snz = DBLE(iz-1) * A2 + vacume_layer

                 else if (icrema == 2) then ! lower free layer
                        
                    snx = DBLE(ix-1) * A0*3.0d0 &
                         & + base_latt(1,i) &
                         & + shift_latt(1,mod(iz-1 +1,3))
                    sny = DBLE(iy-1) * A1*4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,mod(iz-1 +1,3))
                    snz = DBLE(iz-1 +1) * A2 + vacume_layer

                 else if (icrema == 3) then ! upper free layer
                 ...
                 else if (icrema == 4) then ! upper fix layer
                        
                    snx = DBLE(ix-1) * A0*3.0d0 &
                         & + base_latt(1,i) &
                         & + shift_latt(1,mod(iz-1+16,3))
                    sny = DBLE(iy-1) * A1*4.0d0 &
                         & + base_latt(2,i) &
                         & + shift_latt(2,mod(iz-1+16,3))
                    snz = DBLE(iz-1+16) * A2 &
                         & + upper_AU_z
...
!-------- Locate molecules of poly (for arranging thiol) --------
  ...
  do icrepo = 1, ncrecorpo
     ...
     if ((ipoly == 1) .or. (ipoly == 2)) then
        ymaxpo(ipoly) = ymaxpo(ipoly) / 2
        xmaxpo(ipoly) = xmaxpo(ipoly) / 2
        baseloop = 4
     else
        baseloop = 1
     end if
...
!                    --- make gravity coordinate ---
                 ...
                 if (ipoly == 1)  then ! lower thiol
                    rxp = DBLE(ix-1) * A0*3.0d0 &
                         & + base_latt_S1(1,i)
                    ryp = DBLE(iy-1) * A1*4.0d0 &
                         & + base_latt_S1(2,i)
                    rzp = lower_S
...
!                       --- make rotational matrix ---
                    ...
                    rzp = genrand_res53() * oct_layer_z + lower_oct_z
