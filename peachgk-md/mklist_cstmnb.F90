!*******************************
!*  mklist_cstmnb.f90 Ver.1.2  *
!*      for peachgk_md.f       *
!*            by G.Kikugawa    *
!*******************************
! Time-stamp: <2015-01-22 00:53:59 gota>
...
!     -- misc --
  ...
  integer:: cstmnb1, cstmnb2
  integer:: itype, jtype

! MPI
  integer:: nlistcstmnball_lcl ! local variable nlistcstmnball
  integer:: nlistZPcond1all_lcl ! local variable nlistZPcond1all
  integer:: nlistZPcond2all_lcl ! local variable nlistZPcond2all
!  integer:: nZPcond1_index_gbl ! gbl variable nZPcond1_index
!  integer:: nZPcond2_index_gbl ! gbl variable nZPcond2_index
...
!---- Generate NB list for ZP cond (mirror force) interaction

  IF ((mod(current_step,nstep_bookZPcond(1)) == 0) .or. &
       & (nstep_bookZPcond(1) == 1)) THEN

     sqcut = rcut_bookZPcond(1) * rcut_bookZPcond(1)
...
!---- MAKE NONBONDED LIST FOR mirror plane 1 & 2

!    - CHECK IF ATOM(i) is close to mirror image associated with plane 1
...
!      DO i = 1, nZPcond
     icut = 0
     nlistZPcond1all_lcl = 0

     looplast = nZPcond

     DO cstmnb1 = loopinit, looplast, loopstep ! loop over nZPcondlist(cstmnb1)

        i = nZPcondlist(cstmnb1)
        itype = atmindex(i)

        itmp = 0               ! temporal number of bookkeeped atoms
                               ! of atom(i)


        DO cstmnb2 = 1, nZPcond     ! loop over nZPcondlist(cstmnb2)
           j = nZPcondlist(cstmnb2)
           jtype = atmindex(j)

!          - CHECK IF ATOM(i) is close to ATOM(j) -

           rij(1:2) = atmcor(1:2,i) - atmcor(1:2,j)
           rij(3) = atmcor(3,i) - (2.0d0*para_ZPint_p1(1) - atmcor(3,j))
                                        ! j is in image position

!          - periodic boundary -

           rij(1:2) = rij(1:2) - box(1:2) &
                &              * anint(rij(1:2)*box_inv(1:2))
                               ! P.B.C. only for x and y directions
        ...
        END DO                    ! end of loop over nZPcondlist(cstmnb2)

        iitmp = itmp           ! = itmp - excluded atoms

!       - transfer data of list_tmp_i to cstmnb_list

        ZPcond1_indexall(cstmnb1) = icut + 1

        IF (iitmp == 0) then

!          - atom(i) has no neighbor but still 0 is put to cstmnb_list -

           ZPcond1_listall(icut+1) = 0
           icut = icut + 1
           nlistZPcond1all_lcl = nlistZPcond1all_lcl + 1
...
!          - atom(i) has at least one neighbor -
           ...
           do j = 1, itmp
              if (list_tmp_i(j) > 0) then
                 jtmp = jtmp + 1
                 ZPcond1_listall(icut+jtmp) = list_tmp_i(j)
              end if
           end do
           icut = icut + iitmp
           nlistZPcond1all_lcl = nlistZPcond1all_lcl + iitmp

        END IF

     END DO                    ! end of loop over nZPcondlist(cstmnb1)

!    - end of loop -

     nlistZPcond1all = icut
!!! non-existence last loop index cstmnb1
     ZPcond1_indexall(cstmnb1) = nlistZPcond1all + 1

#if defined(MPI)
     call mpi_reduce(nlistZPcond1all_lcl,nlistZPcond1all,1,MPI_INTEGER, &
          &          MPI_SUM,0,MPI_COMM_WORLD,ierror)
#endif

!    - CHECK IF ATOM(i) is close to mirror image associated with plane 2
...
!          - CHECK IF ATOM(i) is close to ATOM(j) -
           ...
           rij(1:2) = atmcor(1:2,i) - atmcor(1:2,j)
           rij(3) = atmcor(3,i) - (2.0d0*para_ZPint_p2(1) - atmcor(3,j))
                                        ! j is in image position
...
!       - transfer data of list_tmp_i to cstmnb_list
        ...
        ZPcond2_indexall(cstmnb1) = icut + 1

        IF (iitmp == 0) then

!          - atom(i) has no neighbor but still 0 is put to cstmnb_list -

           ZPcond2_listall(icut+1) = 0
           icut = icut + 1
           nlistZPcond2all_lcl = nlistZPcond2all_lcl + 1
...
!          - atom(i) has at least one neighbor -
           ...
           do j = 1, itmp
              if (list_tmp_i(j) > 0) then
                 jtmp = jtmp + 1
                 ZPcond2_listall(icut+jtmp) = list_tmp_i(j)
              end if
           end do
           icut = icut + iitmp
           nlistZPcond2all_lcl = nlistZPcond2all_lcl + iitmp
...
!    - end of loop -
     ...
     nlistZPcond2all = icut
!!! non-existence last loop index cstmnb1
     ZPcond2_indexall(cstmnb1) = nlistZPcond2all + 1

#if defined(MPI)
     call mpi_reduce(nlistZPcond2all_lcl,nlistZPcond2all,1,MPI_INTEGER, &
          &          MPI_SUM,0,MPI_COMM_WORLD,ierror)
#endif

     if (irank == 0) then
        write(6,997) nlistZPcond1all,nlistZPcond2all
     end if

997  format(/3x,'ZP cond list updated: ',i10,', ',i10)

  END IF

!---- Generate NB list for ZP aniso & iso interaction

  IF ((mod(current_step,nstep_bookZPaniso(1)) == 0) .or. &
       &  (nstep_bookZPaniso(1) == 1)) THEN

     sqcut = rcut_bookZPaniso(1) * rcut_bookZPaniso(1)
...
!          - transfer data of list_tmp_i to cstmnb_list
           ...
           cstmnb_indexall(cstmnb1) = icut + 1
...
!       - end of loop -
        ...
        nlistcstmnball = icut
!!! non-existence last loop index cstmnb1
        cstmnb_indexall(cstmnb1) = nlistcstmnball + 1
...
!       - cell count
        cellcount(1:3) = INT(box(1:3) / rcut_bookZPaniso(1))
