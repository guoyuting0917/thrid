!*****************************
!*  mklist_cell.f Ver.1.5    *
!*      for peachgk_md.f     *
!*            by G.Kikugawa  *
!*****************************
! Time-stamp: <2015-01-21 23:12:21 gota>
...
!     - cell count
  cellcount(1:3) = INT(box(1:3) / rcut_book)
...
!---- Cell-index registration
  ...
  write(6,*) 'DEBUG: cellindex for vdw+elc interaction'
...
!                    - CHECK IF ATOM(i) is close to ATOM(j) -
                 ...
                 rij(1) = atmcor(1,i) - atmcor(1,j)
                 rij(2) = atmcor(2,i) - atmcor(2,j)
                 rij(3) = atmcor(3,i) - atmcor(3,j)
